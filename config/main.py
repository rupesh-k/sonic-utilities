...
#
# Check if port is already configured as mirror source port
#
def interface_is_mirror_port(mirror_table, interface_name):
...
def validate_mirror_session_config(config_db, session_name, dst_port, src_port, direction):
    ...
    if dst_port is not None:
        ...
        if interface_is_mirror_port(mirror_table, dst_port):
    ...
    if src_port is not None:
        for port in src_port.split(","):
            ...
            if dst_port is not None and dst_port == port:
                ...
                return False
            if interface_is_mirror_port(mirror_table, port):
...
def erspan(session_name, src_ip, dst_ip, dscp, ttl, gre_type, queue, policer, src_port, direction):
    ...
    if not namespaces['front_ns']:
        config_db = ConfigDBConnector()
        config_db.connect()
        if validaanyte_mirror_session_config(config_db, session_name, None, src_port, direction) is False:
            return
